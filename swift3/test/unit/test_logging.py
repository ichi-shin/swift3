...
from swift3.subresource import ACL, LoggingStatus, encode_subresource
...
class TestSwift3Logging(Swift3TestCase):
    ...
    def setUp(self):
        ...
        headers = encode_subresource('container', 'logging',
                                     LoggingStatus(conf_xml))
        headers.update({'x-container-sysmeta-swift3-acl':
                        '["test:tester"]'})
        ...
        self.app.register('HEAD', '/v1/AUTH_test/mybucketlogs',
                          swob.HTTPNoContent,
                          encode_subresource('container', 'acl', ACL(acl_xml)),
                          None)
    ...
    def test_log_delivery_error(self):
        ...
        with nested(patch('os.stat'),
                    patch('os.unlink')) as (m, _):
    ...
    def test_log_delivery_translate_line(self):
        with patch('swift3.log_delivery.wsgi'):
            x = LogDelivery({})
            log = x.translate_line(sample_log)
            self.assertEquals(log, ('bucket',
    ...
    def test_log_delivery_head_object(self):
        ...
        self.app.register('HEAD', '/v1/AUTH_test/bucket/object',
                          ...
                          {'x-container-sysmeta-swift3-timestamp': 0,
                           'x-container-sysmeta-swift3-owner':
                           'test:tester'},
                          None)
        x = LogDelivery(conf)
        resp = x.head_object('AUTH_test', 'bucket', 'object')
        self.assertEquals(204, resp.status_int)  # pylint: disable-msg=E1101
    ...
    def test_bucket_logging_owner_error(self):
        bucket_name = 'junk'
        req = Request.blank('/%s?logging' % bucket_name,
                            environ={'REQUEST_METHOD': 'GET'},
